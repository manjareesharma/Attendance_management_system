...
def open_lms():
    startfile("lms\\src\\html\\authentication-login.html")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self.add_details_frame, text="Course:", font=("", 16)).grid(
            row=2, column=0, padx=(15, 2), pady=10
        )
        self.student_course_text = tk.StringVar()
        ctk.CTkEntry(
            self.add_details_frame, width=400, textvariable=self.student_course_text
        ).grid(row=2, column=1, padx=(10, 10), pady=10)

        ctk.CTkLabel(self.add_details_frame, text="Year:", font=("", 16)).grid(
            row=3, column=0, padx=(15, 2), pady=10
        )
        self.student_year_text = tk.StringVar()
        ctk.CTkEntry(
            self.add_details_frame, width=400, textvariable=self.student_year_text
        ).grid(row=3, column=1, padx=(10, 10), pady=10)
        ...
        self.student_treeview["columns"] = ("Student ID", "Name", "Course", "Year")
        ...
        self.student_treeview.column("Course", anchor="w", stretch=True)
        self.student_treeview.column("Year", anchor="w", stretch=True)
        ...
        self.student_treeview.heading("Course", text="Course", anchor="w")
        self.student_treeview.heading("Year", text="Year", anchor="w")
    ...
    def add_student_button_event(self):
        ...
        if (
            ...
            or self.student_course_text.get() == ""
            or self.student_year_text.get() == ""
        ...
        db.insert(
            ...
            self.student_course_text.get(),
            self.student_year_text.get(),
            img_bytes,
            face_encoding_bytes,
        )
        del db
    ...
    def delete_student_record(self):  # sourcery skip: use-named-expression
        selection = self.student_treeview.focus()
        if selection:
            id, name, course, year = self.student_treeview.item(selection, "values")
            if messagebox.askyesno(
                "Delete Student", f"Are you sure you want to delete {name} ({id}) {course} {year}?"
            ):
                db = Database("student.db")
                db.remove(id)
                del db
